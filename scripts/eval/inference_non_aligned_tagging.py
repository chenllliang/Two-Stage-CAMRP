...
def load_inference_dataset(token_path):
    ...
    for tok in tokens:
        cur_tok = tok.strip().split("\t")[1].split()
...
NODE_PATH=input_dir
inference_dataset = load_inference_dataset(NODE_PATH)
inference_dataset = inference_dataset.map(tokenize_and_align_labels,batched=True)
...
model = AutoModelForTokenClassification.from_pretrained(model_dir, num_labels=185)

metric = load_metric("seqeval")

#185 classes
tag_path="../../preprocessed/non_aligned_concept_tagging/train.tag.extra_nodes_dict"
tags_index={}
...
predictions, labels, _ = trainer.predict(inference_dataset)
predictions = np.argmax(predictions, axis=2)
...
with open(out_dir+".non_aligned","w") as f:
