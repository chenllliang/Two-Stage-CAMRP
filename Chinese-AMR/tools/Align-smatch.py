...
def normalize(item):
    """
    lowercase items that are about to be compared
    """
    if isinstance(item, tuple) == 1:
        l = [i.lower() for i in item]
        return tuple(l)
    else:
        return item.lower()


def split_frame(item):
    """
    drop out frame in predicate concept
    """
    return re.sub(r'-\d\d$', '', item)
    ...
    for instance1_item in instance1:
        ...
        if doinstance:
            for instance2_item in instance2:
                ...
                if normalize(instance1_item[2]) == normalize(instance2_item[2]):
    ...
    if doattribute:
        for attribute1_item in attribute1:
            for attribute2_item in attribute2:
                # if both attribute relation triple have the same value
                if normalize(attribute1_item[2]) == normalize(attribute2_item[2]):
        ...
        for relation1_item in relation1:
            for relation2_item in relation2:
                ...
                if normalize(relation1_item[0]) == normalize(relation2_item[0]) :
                    ...
                    # concepts of the top nodes in two CAMR needs be same
                    # concepts(without frame) of nodes in relation_item needs be same
                    if relation1_item[0] == 'top':
                        if normalize(instance1[node1_index_amr1][2]) != normalize(instance2[node1_index_amr2][2]) \
                            or normalize(instance1[node2_index_amr1][2]) != normalize(instance2[node2_index_amr2][2]):
                            continue
                    else:
                        if split_frame(normalize(instance1[node1_index_amr1][2])) != split_frame(normalize(instance2[node1_index_amr2][2])) \
                            or split_frame(normalize(instance1[node2_index_amr1][2])) != split_frame(normalize(instance2[node2_index_amr2][2])):
                            continue
...
def generate_amr(f1, f2, sid_len):
    """
    ...
    :param sid_len: file handle (or any iterable of strings) to read max length of sentences from
    :return: generator of cur_amr1, cur_amr2 pairs: one-line AMR strings
    """
    while True:
        cur_amr1 = read_tuples.CAMR.get_CAMR(f1, sid_len, smatch=False)
        cur_amr2 = read_tuples.CAMR.get_CAMR(f2, sid_len, smatch=False)
...
def get_amr_match(camr1, camr2, sent_num, justinstance=False, justattribute=False, justrelation=False):
    ...
    node_amr1.rename_node(prefix1)
    # Renaming node to "b1", "b2", .etc
    node_amr2.rename_node(prefix2)
    (instance1, attributes1, relation1) = node_amr1.get_triples()
    (instance2, attributes2, relation2) = node_amr2.get_triples()
    if verbose:
    ...
    (node_mapping, best_match_num) = get_best_match(instance1, attributes1, relation1,
    ...
    if verbose:
        ...
        print("best node mapping", node_mapping, file=DEBUG_LOG)
        print("Best node mapping alignment:", print_alignment(node_mapping, instance1, instance2), file=DEBUG_LOG)
    if justinstance:
...
def score_amr_pairs(f1, f2, leng, justinstance=False, justattribute=False, justrelation=False):
    """
    Score one pair of AMR lines at a time from each file handle
    :param f1: file handle (or any iterable of strings) to read AMR 1 lines from
    :param f2: file handle (or any iterable of strings) to read AMR 2 lines from
    :param leng: file handle (or any iterable of strings) to read max length of sentences from
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Align-smatch calculator")
    parser.add_argument(
        '-f',
        nargs=2,
        required=True,
        type=argparse.FileType('r',encoding='utf-8'),
        help=('Two files containing CAMR tuples. '
              'CAMR tuples in each file are separated by a single blank line'))
    parser.add_argument(
        '-lf',
        required=True,
        type=argparse.FileType('r', encoding='utf-8'),
        help=('A files containing max length of each sentence'))
    parser.add_argument(
        '-r',
        type=int,
        default=4,
