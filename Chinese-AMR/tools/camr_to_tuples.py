    ...
    def parse_AMR_line(line, align_dict):
        ...
        def update_triple(node_relation_dict, relation_align_dict, u, r, v):
            ...
            if u in relation_align_dict and relation_align_dict[u] != []:
                if relation_align_dict[u][-1][0] == r and isinstance(relation_align_dict[u][-1][1], tuple) == 0:
                ...
                if '_' not in anchor:
                    ...
                    for w in split_sequence:
                        if '.' not in w:
                            ...
                            joint_word.append(align_dict[int(w.split('.')[0])][int(w.split('.')[1])-1])
            ...
            if c == "\"":
                ...
                if state == 3:
                    ...
                    if opx_list == []:
                        ...
                        node_opx_dict[cur_node_name].append((opx_list[-1], node_value))
                        opx_list.pop()
                ...
                # at this point we get "x3"
                    ...
                    if cur_relation_name != "":
                        update_triple(node_relation_dict, relation_align_dict, stack[-1], cur_relation_name, node_name)
                elif state == 1:
                    print("Error in parsing AMR" + line[0:i + 1])
                    print("If there is a colon, slash or bracket in the concept, please put double quotation marks on the concept!")
                    return None, None
                state = 2
            elif c == "/":
                if in_quote:
                ...
                if state == 1:
                    ...
                    if cur_relation_name != "":
                        ...
                        if cur_relation_name not in relation_align_dict:
                            update_triple(node_relation_dict, relation_align_dict, stack[-2], cur_relation_name,
                                          node_name)
                        else:
                            relation_align_dict[stack[-2]].append((cur_relation_name, node_name))
                            state = 3
                            continue
                    if node_name not in node_dict:
                        ...
                        if '_' not in node_name:
                            if int(node_name) <= max(align_dict.keys()):
                                node_align_dict[node_name].append(('anchor', node_name))
                        else:
                            node_align_dict[node_name].append(('anchor', id_anchor('x' + node_name)))
                elif state == 2:
                ...
                if len(stack) == 0:
                    print("Unmatched parenthesis at position" + str(i) + "in processing" + line[0:i + 1])
                    return None, None
                # e.g. :arg1() x3)
                ...
                # we get "你" here
                    ...
                    if opx_list == []:
                        ...
                        node_opx_dict[cur_node_name].append((opx_list[-1], node_value))
                        node_dict[cur_node_name] = ''.join([each_name[1] for each_name in node_opx_dict[cur_node_name]])
                        opx_list.pop()
        ...
        for v in node_name_list:
            ...
            if v in relation_align_dict:
                for v2 in relation_align_dict[v]:
                    node_rel_list.append([(v2[1][0], arc_dict[v2[1][0]]), v2[1][1]])
        ...
        result_amr = CAMR(node_name_list, node_value_list, relation_list, attribute_list)
        return result_amr, coref_dict

def main(arguments):
    """
    Main function of CAMR_to_tuple
    """
    print('\t'.join(['句子编号', '节点编号1', '概念1', '同指节点1', '关系', '关系编号', '关系对齐词', '节点编号2', '概念2',
                     '同指节点2']), file=arguments.output)
    print('\t'.join(['sid', 'nid1', 'concept1', 'coref1', 'rel', 'rid', 'ralign', 'nid2', 'concept2', 'coref2'])+'\n',
          file=arguments.output)
    while True:
        sentence = []
        amr_lines, sid, align_dict = CAMR.get_amr_line(arguments.input)
        if not amr_lines:
            pass
        else:
            tuples, coref = CAMR.parse_AMR_line(amr_lines, align_dict)
            for index, rel in enumerate(tuples.relations):
                node = tuples.nodes[index]
                concept = tuples.node_values[index]
                if rel != []:
                    for e_rel in rel:
                        row = [sid]
                        row.append('x'+node)
                        ...
                        for k,v in coref.items():
                            ...
                            row.append('-')
                        if isinstance(e_rel[0], tuple) == 0:
                            row.append(':'+e_rel[0])
                            row.append('-')
                            row.append('-')
                            n2 = e_rel[1]
                            c2 = tuples.node_values[tuples.nodes.index(e_rel[1])]
                            row.append('x'+n2)
                            row.append(c2)
                            for k, v in coref.items():
                                if n2 in v:
                            ...
                            for c in range(len(rel)-1):
                                if sentence[-1-c][7] == 'x'+e_rel[1]:
                                    sentence[-1-c][5] = 'x'+e_rel[0][0]
                                    sentence[-1-c][6] = e_rel[0][1]
                                    break
                            else:
                                print('Wrong relation align: '+sid)
            for i, s in enumerate(sentence):
                if i == len(sentence)-1:
                    print('\t'.join(s)+'\n', file=arguments.output)
                    continue
                print('\t'.join(s), file=arguments.output)
            continue
        break
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="CAMR_to_tuple")
    parser.add_argument(
        '-input',
        required=True,
        type=argparse.FileType('r', encoding='utf-8'),
        help=('A file containing CAMR. '
              'CAMRs are separated by a single blank line'))
    parser.add_argument(
        '-output',
        required=True,
        type=argparse.FileType('w', encoding='utf-8'),
        help=('Tuples converted from CAMR will write into this file. '))
